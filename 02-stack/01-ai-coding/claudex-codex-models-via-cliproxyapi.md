...
Install the CLIProxyAPI release binary on `PATH` and keep its config/client key under `~/.cli-proxy-api/`. Provider login is otherwise launcher-driven: proxy lanes check only the OAuth providers they actually require before starting Claude Code.
...
Quota, cooling-down, rate-limit, and temporary upstream failures do **not** trigger credential replacement. The launcher aborts and reports the provider state instead.
...
The preflight reports this as its own provider state, `quota-unavailable`, and deliberately does **not** offer the transient "launch anyway" bypass, because every request would be refused before it left the machine. The prompt offers three moves: reset the cooldown, switch to a native Anthropic lane, or abort (the default, including with no TTY).
...
|---|---|---|---|
| `cc` (+`y/ry`…) | native Claude | selected tier, guard-locked | Explicit first-party fallback; no proxy preflight |
| `claudex` (+`y/ry`) | GPT-5.6-Sol | GPT-5.6-Sol, guard-locked | Preflights Codex only |
| `claudexm` (+`y/ry`) | GPT-5.6-Sol | cross-provider, no default or lock | Preflights Codex + Claude; session-scoped `sonnet-worker` selects Claude Sonnet 5 |
| `claudexma` (+`y/ry`) | GPT-5.6-Sol | direct Sol/Astra, no default or lock | OpenAI/Codex-only; model-neutral workers inherit Sol, direct Astra requires `approve-agent`, and declared Astra model-budget Workflows are one-run approvable; preflights Codex only |
| `astrax` (+`y/ry`) | GPT-6 Astra | GPT-5.6-Sol, guard-locked | OpenAI-only premium lane; both use the weekly Codex pool; preflights Codex only |
| `astraxm` (+`y/ry`) | GPT-6 Astra | direct Sol/Sonnet/Opus/Fable, no default or lock | Fable and model-neutral Astra direct workers require `approve-agent`; a Workflow declaring an approved model-budget envelope (hard total cap plus per-model ceilings labeling Astra and/or Fable) is one-run approvable; preflights Codex + Claude |
| `fablex` (+`y/ry`) | Fable 5.1 | GPT-5.6-Sol, guard-locked | Active exact `claude-fable-5-1` route; preflights Claude + Codex |
| `fablexm` (+`y/ry`) | Fable 5.1 | direct Sol/Sonnet/Astra, no default or lock | Astra and model-neutral Fable direct workers require `approve-agent`; a Workflow declaring an approved model-budget envelope (hard total cap plus per-model ceilings labeling Astra and/or Fable) is one-run approvable; preflights Claude + Codex |
| `opusx` (+`y/ry`) | Opus 5 | GPT-5.6-Sol, guard-locked | Preflights Claude + Codex |
| `opusxm` (+`y/ry`) | Opus 5 `[1m]` | explicit cross-provider; no default or lock | Normal `x`/Portagenty lane; ordinary workers inherit Opus and session-scoped `sol-worker` selects GPT-5.6-Sol |
| `claudexo` (+`y/ry`) | GPT-5.6-Sol | Opus 5, guard-locked | Preflights Codex + Claude; cost-inverted fan-out |
...
For an exact UUID, `ccx` reads the latest real top-level assistant model from pconv's JSONL source, ignoring worker sidechains and internal markers such as `<synthetic>`. A compatible session resumes directly. A model mismatch uses Claude Code's supported `--fork-session -r <uuid>` path: the target launcher model gets a new UUID with full retained history, while the source transcript remains byte-for-byte untouched. If the installed Claude Code lacks `--fork-session`, or the recorded model cannot be established safely, `ccx` aborts before launch and asks for `handoff` or `fresh` instead of guessing. The bounded pconv handoff remains an explicit recovery choice for oversized or damaged conversations; it is no longer the normal model-migration path.
...
CLIProxyAPI `v7.2.151` advertises and routes exact `claude-fable-5-1` and `gpt-6-astra` IDs. The active proxy launchers never silently downgrade Fable 5.1 to Fable 5 or Astra to another GPT model. `fablex`, `astrax`, and `claudex` run model-neutral Workflows on guard-locked Sol without extra approval. `fablexm` and `astraxm` are premium-main direct-worker steering lanes, while `claudexma` keeps Sol as main and exposes Astra explicitly; each can run a Workflow declaring an approved Astra/Fable model-budget envelope alongside the existing per-worker `approve-agent` path.
...
| **Codex pool exhausted** | explicit native `cc*` or Fable lane | Claude only |
...
| Either pool running low | avoid mixed proxy lanes that require both providers | — |
| Manual native-Claude model control | `ccrym` / `fablem` | Claude only; Sol is not reachable |

Corollaries worth internalizing:

- Native lanes never touch OpenAI. `claudex` is pure Sol, `claudexma` is flexible Sol/Astra, and `astrax` is Astra + Sol; all three preflight Codex only. `opusx`, `opusxm`, `claudexo`, `astraxm`, `fablex`, and `fablexm` intentionally depend on both provider routes and preflight both.
- `claudexm` also preflights both routes because its Sol main can delegate explicitly to Claude workers; use `claudexma` when the worker surface must remain OpenAI/Codex-only.
- Flexible premium lanes pair per-worker `approve-agent` steering with one-run Workflow approval: use `astraxm` or `fablexm` for direct per-worker steering or a declared model-budget Workflow envelope, and use `astrax` or `fablex` when a model-neutral Workflow should run on guard-locked Sol workers instead.
- The convenience default is still route-sensitive: `opusxm`, `claudexm`, and every mixed premium lane require both providers to be healthy. On a 429, authentication, cooling-down, routing, or unexpected-model failure, stop rather than retry-loop; relaunch through an explicit native or single-provider lane only when that is the intended fallback.
...
- `exactModelId` is one of the six supported IDs: `gpt-6-astra`, `gpt-5.6-sol`, `claude-fable-5-1`, `claude-opus-5`, `claude-sonnet-5`, `glm-flash`. Native (non-proxy) routes support Claude models only — `gpt-*` and `glm-flash` are proxy-only and a native-route budget declaring one is denied. Reaching all five requires the session actually be routed through the configured allowed CLIProxyAPI origin — currently `http://127.0.0.1:8317`, normalized and checked as an exact match, not "any loopback/localhost server." A pinned/locked lane rejects this explicit model-budget form entirely.
