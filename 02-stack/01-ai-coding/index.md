...
### Surface boundary: local execution, local authority

Terminal-first remains the default, but the frontend is not the source of truth. The official VS Code extension is worth evaluating as a local graphical layer for diffs, multiline rendering, copy ergonomics, and parallel session tabs. Remote Control stays off unless its Anthropic-mediated synchronization boundary is separately accepted; tailnet-constrained remote desktop is the preferred graphical remote-access direction.

See [Claude Code UI surfaces and trust boundaries](./claude-code-ui-trust-boundaries.md) for the CLI, extension, Remote Control, cloud execution, and Portagenty boundary in one decision guide.
...
## TUI rendering — turn on fullscreen (no-flicker)

Claude Code's default TUI renders the conversation inline and repaints the whole region each frame — visible flicker, worst on narrow panes and over SSH/multiplexers. Since v2.1.110 there's an alt-screen + per-frame-diff renderer that only redraws changed cells, anchors the input box at the bottom, and eliminates the flicker.

Turn it on once and it persists (the `/tui` command writes the setting and relaunches):

```
/tui fullscreen      # enable — saved to config, survives restarts
/tui default         # revert if needed
```

Equivalent older mechanism: `CLAUDE_CODE_NO_FLICKER=1` env var (pre-2.1.110). If both are set they can conflict — prefer the `/tui` setting and unset the env var.

Tradeoffs / caveats:
- Uses the alt-screen, so terminal-native scrollback and copy behavior change (scroll within Claude Code, not the emulator's buffer).
- Reported CJK-text-on-copy corruption in some terminals with the no-flicker renderer; `/tui default` is the fallback if non-Latin text comes out garbled.
- Recommended default for this stack — flicker-free + pinned input is strictly better for long agent sessions in a multiplexer.
...
- [`context-window-engineering-and-delegation.md`](./context-window-engineering-and-delegation/) — **start here for context/delegation decisions**: advertised vs routed vs managed windows, harness metadata fallbacks, compaction strategy, and quality/quota/context as joint delegation axes
- [`portable-agent-permissions.md`](./portable-agent-permissions/) — natural-language request → exact prepared terms → genuine-human Approve/Deny UI for bounded future premium-worker envelopes; exact one-use grants remain the default and fallback
- [`browser-automation-pilot.md`](./browser-automation-pilot/) — persistent permission-bound visible browser sessions across observe → act → observe calls, with exact-origin scope, stale-reference rejection, masked screenshots, and owned Stop cleanup; includes the old synthetic runner as historical evidence
- [`desktop-computer-use-pilot.md`](./desktop-computer-use-pilot/) — permission-bound GUI computer use on a KDE Wayland host: the `computer-use-linux` MCP server behind a `desktop-bridge` proxy, a human-approved `desktop_session` grant, window-targeted captures, bounded pointer arguments, and owned Stop cleanup
- [`claude-code-cloud-sessions.md`](./claude-code-cloud-sessions/) — Anthropic-hosted cloud sessions: what they lose versus the local stack (proxy lanes, guard, hooks, tier-3 files), which tasks fit, how to start one, and how the one-time credit is spent before plan usage
- [`claudex-codex-models-via-cliproxyapi.md`](./claudex-codex-models-via-cliproxyapi/) — the deliberate, eyes-open exception to the wrapper caution above: Codex/GPT models inside the Claude Code harness via a self-hosted loopback proxy (experimental lane, ban-risk acknowledged, context-window wedge documented)
