...
description: Tailscale-backed terminal, browser, chat-native, and graphical routes for reaching local agent sessions from another device without defaulting to public exposure.
stratum: 2
status: research
sidebar:
  order: 1
...
- **Authorization recovery:** an `additional check` is an expected browser reauthentication challenge from Tailscale SSH, not an `sshd` failure. Keep the original `tailscale ssh <linux-user>@<host> '<bounded command>'` process alive, surface its login URL, let the browser check complete, and verify `id`/`hostname` on that same process. Restart services only when status or ping proves a separate fault.
...
### Current Android baseline: Termux
...
- **Observed limits:** terminal scrollback and touch copy/paste remain poor for long agent responses, and device-specific battery use can be high. Keep Termux as a capable repair shell even if another surface becomes the primary mobile controller.

The mobile client and control-plane decision is now intentionally open. See [Mobile agent access surfaces](./mobile-agent-access-surfaces.md) for ConnectBot/SSH, Mosh clients, Claude Code Remote Control, T3 Code over Tailscale, remote desktop, and read-only browser routes.
...
## Graphical remote access: one evaluation path

For clean multiline rendering, structured diffs, one-click copy, and session tabs, full remote desktop remains one option alongside the local-web T3 Code route and account-mediated Claude Remote Control:

```text
Phone → Tailscale → RustDesk → local graphical Claude Code surface
```

The workstation remains the execution and filesystem host, and the tailnet remains the intended remote-access perimeter. This does not make model inference local: prompts and selected context still traverse the configured provider. RustDesk must be separately verified to use the intended tailnet path without a public relay or newly exposed port before adoption.

Claude Code Remote Control is architecturally different. Execution remains local, but the control connection runs through Anthropic infrastructure and stores synchronized activity on Anthropic servers while connected. T3 Code takes another route: a local server owns provider processes and exposes authenticated web/mobile clients over an explicitly configured LAN, tailnet, or HTTPS endpoint. Keep both off until their distinct access boundaries are reviewed. See [Claude Code UI surfaces and trust boundaries](../01-ai-coding/claude-code-ui-trust-boundaries.md) and [Mobile agent access surfaces](./mobile-agent-access-surfaces.md).
...
- [Mobile agent access surfaces](./mobile-agent-access-surfaces.md) — terminal, Mosh, Remote Control, T3 Code, remote-desktop, and read-only browser options with trust and UX tradeoffs
...
- [Tailscale file-share — six flavors](../patterns/tailscale-file-share-four-flavors.md) — decision framework for picking between Taildrop / SMB / `tailscale serve` / `tailscale funnel` / **Tailscale Drive (alpha)** / **Filebrowser**, including Windows-specific gotchas (Taildrop broken per GH #14393, WSL2-inbound asymmetric-NAT failure, `nslookup` lies on Windows)
- [Tailscale HTTPS three levels](../patterns/tailscale-https-three-levels.md) — when you need *persistent* HTTPS, not ad-hoc file share
