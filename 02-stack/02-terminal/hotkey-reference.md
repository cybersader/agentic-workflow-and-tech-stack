...
The profile in this stack starts in **locked mode** so application keys pass through, and it disables pane frames for clean multiline copying. Press `Ctrl+G` to unlock before using the mode keys below. To show or hide frames at runtime, press `Ctrl+G` → `Ctrl+P` → `Z`.

| Mode | Enter with | What it does |
|---|---|---|
| Locked | Stack profile default; `Ctrl+G` re-enters it | Pass every application key through; `Ctrl+G` unlocks |
| Normal | `Ctrl+G` from locked mode | Type into the shell and access Zellij shortcuts |
...
| Toggle zoom | `Alt+F` |
| Toggle pane frames (stack profile) | `Ctrl+G` → `Ctrl+P` → `Z` |
