...
#### Pane frames: off by default

This stack sets `pane_frames false`. Borders are useful when identifying the active pane, reading pane titles, or finding split and resize boundaries, but they also get swept into wrapped or multiline terminal selections—especially through narrow remote and mobile clients. Clean copying is the higher-value default; frames remain an on-demand navigation aid.

The profile starts in locked mode, so the runtime toggle is `Ctrl+G` → `Ctrl+P` → `Z`: unlock Zellij, enter pane mode, then toggle pane frames. The default applies to new sessions; existing sessions can use the toggle immediately.
...
#### Research boundary: control plane, transport, and supervision

Portagenty currently resolves declared sessions and controls their multiplexer lifecycle. It is **not yet a message bus or resource supervisor**: it does not deliver addressed envelopes, queue acknowledgements, attribute child processes, report cgroup pressure, or provide granular process actions. Its stable workspace UUID plus exact declared session name is nevertheless a plausible logical address for both future capabilities.

The current research keeps those concerns separate. [Parallel Agent Coordination](../../agent-context/zz-research/2026-04-25-parallel-agent-coordination-findings.md) defines the proposed transport-neutral address, envelope, adapter, and receipt boundary. [Agent Resource Supervision and Control Plane](../../agent-context/zz-research/2026-08-21-agent-resource-supervision-and-control-plane.md) covers reuse-safe process/cgroup attribution, pressure observability, and a narrow-to-wide termination ladder. Neither note promises terminal input injection, a GUI, or an implemented daemon.
