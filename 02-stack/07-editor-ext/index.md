...
### Official Claude Code extension (evaluate as a local GUI layer)

The official extension is different from adding a second AI assistant: it is a graphical surface for Claude Code itself. It provides session history, multiple conversation tabs or windows, structured diff review, file/line mentions, and terminal-output references, while sharing settings and conversation history with the CLI.

Use it as a view over the same local projects and sessions—not as a replacement for Git, Portagenty, project instructions, or filesystem knowledge. Keep its optional Remote Control setting disabled unless that separate cloud-synchronization boundary is explicitly approved.

Decision guide: [Claude Code UI surfaces and trust boundaries](../01-ai-coding/claude-code-ui-trust-boundaries.md). Official product details: [Use Claude Code in VS Code](https://code.claude.com/docs/en/vs-code).
