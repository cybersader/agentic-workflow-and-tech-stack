...
## Agent workload supervision — research

Multiple agent sessions can leave a Linux workstation responsive but severely degraded through CPU contention, reclaim/swap pressure, I/O stalls, or descendant-process growth. cgroup v2 and systemd scopes are the aggregation substrate worth evaluating because a logical workload usually spans more than one PID. Process GUIs such as Plasma System Monitor, Mission Center, or Resources and terminal views such as `systemd-cgls`/`systemd-cgtop` are complementary; none currently provides the complete Portagenty-session-to-cgroup control plane.

Utilization alone is not enough. Linux pressure stall information (PSI) helps distinguish a busy workload from one that is actively delaying other work. The research target is a local view that combines stable session attribution, expandable process trees, aggregate CPU/memory/swap/I/O pressure, and confirmation-gated actions from one attributed child through the full session scope. This has not graduated into an installable pattern or GUI recommendation; see [Agent Resource Supervision and Control Plane](../../agent-context/zz-research/2026-08-21-agent-resource-supervision-and-control-plane.md).
...
- [`../05-homelab/self-hosted-deployment-platforms.md`](../05-homelab/self-hosted-deployment-platforms.md) — self-hosted deployment platforms (migrated into 05-homelab)
