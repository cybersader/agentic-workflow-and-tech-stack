...
## Gotcha: text & clipboard uploads default to public Pastebin

The image path above is private, but ShareX has a **separate text uploader**, and out of the box it points at **public Pastebin**. If you ever use the "Upload text" action — or trigger a `ClipboardUpload` hotkey while the clipboard holds *text* instead of an image — ShareX silently pushes that content to Pastebin. That bypasses the entire Tailscale-gated design for anything that isn't an image.

:::caution
There is **no per-service "disable Pastebin" switch** in ShareX. Built-in destinations always exist in the Destinations menu; you control exposure by which destination is *selected*, not by removing a service. The global **"Disable upload"** toggle exists but kills *all* uploading (including Zipline), so it's too broad. The real fix is to make sure **no destination setting anywhere resolves to Pastebin** and instead routes text to your own Zipline.
:::

**Where the settings live** — ShareX config is JSON, *not* a registry key (`%USERPROFILE%\Documents\ShareX\`):

- `ApplicationConfig.json` → `"TextDestination"` — the global default. Set to `"CustomTextUploader"` so text routes to the Zipline custom uploader instead of Pastebin.
- `UploadersConfig.json` → the Zipline custom uploader's `"DestinationType"` must include `TextUploader` (e.g. `"ImageUploader, TextUploader, FileUploader"`), or ShareX won't treat it as eligible for text and the selection above does nothing.
- `HotkeysConfig.json` → **every hotkey carries its own `"TextDestination"`.** Hotkeys with `"UseDefaultDestinations": false` — which the clip2path clipboard hotkeys use — **ignore the global default** and fire their own value. This is the easy one to miss: you fix the global default, feel safe, and still leak via a clipboard hotkey. Set every occurrence to `"CustomTextUploader"`.

:::note Edit safely
ShareX **rewrites these JSON files on exit**, so a normal close will overwrite hand-edits. Force-kill ShareX (`taskkill /F /IM ShareX.exe`) *before* editing, then relaunch so it loads the new config.
:::

**Verify** — no destination should resolve to Pastebin (expect zero matches):

```bash
grep -rn '"[A-Za-z]*Destination": "Pastebin"' "/mnt/c/Users/<you>/Documents/ShareX/"*.json
```

The leftover `PastebinSettings` block in `UploadersConfig.json` is harmless as long as it holds no credentials (username/userkey `null`) and nothing points at it — it's just the dormant settings object for a service you no longer use.

:::note Why this matters for the agentic workflow
clip2path exists so agents consume shared content from **private, Tailscale-gated URLs** rather than public services. A clipboard-text upload silently landing on Pastebin breaks that guarantee for text exactly the way a public Zipline would for images.
:::
