...
The bug from [2026-04-23-stale-sessions-index-bug](/agentic-workflow-and-tech-stack/agent-context/zz-research/2026-04-23-stale-sessions-index-bug/) **recurred today, 2026-04-26**, on the same project (mcp-workflow-and-tech-stack). The 2026-04-25 fix landed `pconv doctor` and `pconv rebuild-index` as portaconv v0.1.0 subcommands — they exist and they work — yet the bug surfaced fresh and required manual recovery.
...
| 2026-04-26 (morning) | WSL terminal close (suspected) | `pconv dump --tail 200` + `mv sessions-index.json` + new `ccrh` shell helper | ~5 min (well-trodden path) |
| 2026-04-26 (evening) | WSL terminal close (third recurrence in one day) | manual `pconv dump --full-results` + `mv sessions-index.json`; `ccah` (one-shot archive + heal) shipped same session | ~5 min recovery (manual); next bite uses `ccah` |
| _next_ | _track here_ | _ideally `ccah` from project root_ | _< 5 sec_ |

Append future occurrences as they happen. With `ccah` in place, the next recurrence should land near-zero latency — one command produces both the durable md archive and the index heal, no manual sequence.
...
- [2026-04-23 stale-sessions-index bug](/agentic-workflow-and-tech-stack/agent-context/zz-research/2026-04-23-stale-sessions-index-bug/) — the original investigation
- [2026-04-23 upstream comment 25032](/agentic-workflow-and-tech-stack/agent-context/zz-research/2026-04-23-upstream-comment-25032/) — what got pushed upstream
- [research/learnings/2026-04-23-stale-sessions-index-detection-and-recovery.md](/agentic-workflow-and-tech-stack/research/learnings/2026-04-23-stale-sessions-index-detection-and-recovery/) — distilled insight, generalizes past Claude Code
- [02-stack/patterns/claude-code-session-recovery.md](/agentic-workflow-and-tech-stack/stack/patterns/claude-code-session-recovery/) — practitioner decision tree
...
- [research/zz-challenges/02-claude-code-conversation-fragmentation.md](/agentic-workflow-and-tech-stack/research/zz-challenges/02-claude-code-conversation-fragmentation/) — sibling tracked challenge for shape comparison
