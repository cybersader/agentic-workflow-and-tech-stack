...
## Session 6: cc-archive-and-heal — second-recurrence-of-the-day prompts a one-shot recovery command

Third stale-sessions-index recurrence today (this evening). The morning recovery (Session 1) had been manual: `pconv dump --tail` → `mv sessions-index.json`. Tonight, after a `pconv doctor` + manual `pconv dump` + manual rename round-trip yet *again*, the friction tipped — frustration was the user's word. Decision: ship a single command that does archive + heal in one shot, tied to the project's encoded-cwd dir, ready for next recurrence.

- `profiles/bashrc-snippets/claude-code-helpers.sh` — added `cc-archive-and-heal` function (alias `ccah`). Sits next to the morning's `cc-resume-here` / `ccrh`; same encoded-cwd derivation. Two operations: (a) `pconv dump <uuid> --full-results --include-system-events` of the most-recent jsonl into `~/.claude/archives/<encoded-cwd>/<ts>-<uuid>.md` (durable record, out of tree, private), (b) rename any live `sessions-index*.json` to `.bak-<ts>-stale` (the manual heal recipe, scripted). Idempotent — second run produces a fresh archive with new timestamp; heal is no-op when no live index exists. Bash `-n` clean.
- Verified end-to-end: sanity run from `/tmp` bails with "no Claude Code project dir" + exit 1; real run from this project dir produced a 251K-line / 15 MB archive of the active 97d7b58b session; jsonl count unchanged (21 → 21); `sessions-index*` files unchanged (heal correctly reported no live index since this morning's heal already ran).

**Why a light-wipe instead of full nuke:** the user picked "heal index only, keep jsonls" — pconv can re-render any jsonl into md anytime, so deleting them costs durable history without buying anything. The one-time archive is the durable handoff; the index rename is the cheap fix for the actual bug.

**Follow-up captured (not done):** consider a `pconv archive-and-heal` (or `pconv heal --archive`) subcommand in portaconv that does the same two operations natively in Rust, removing the bashrc dependency. The bashrc helper is the immediate ship; the portaconv subcommand is the portable next step. Logged here so it doesn't get lost.

**Decision deferred:** `claude` self-heal at launch (bashrc wrapper that auto-runs `pconv doctor --auto-fix` before exec, or a SessionStart hook in `.claude/settings.json`) is still the right "stop the bug from disrupting flow at all" answer — but separate ship. Today's friction was post-bug recovery, not launch-time prevention.
