...
- **Environment:** Claude Code v2.1.118, Zellij 0.43.1, WSL2 on Windows, `TERM=xterm-256color`. Narrow pane via phone-over-Tailscale-SSH reattach (see [cross-device SSH pattern](../../../02-stack/patterns/cross-device-ssh.md)).
...
- **Stack pattern (tier 2):** belongs as a "gotchas" subsection of [cross-device SSH](../../../02-stack/patterns/cross-device-ssh.md) rather than its own pattern — the bug is situational to that pattern's narrow-pane workflow, not a standalone decision tree. Fold in when the next pass through that doc lands.
...
## Update 2026-06-13 — fullscreen renderer shipped (may address the class)

Claude Code v2.1.110 shipped an alt-screen + per-frame-diff renderer (`/tui fullscreen`, or the older `CLAUDE_CODE_NO_FLICKER=1`) that replaces the repaint-the-whole-region-each-frame approach. This fixes *flicker* definitively. **Whether it also fixes the narrow-pane mid-word-wrap corruption documented here is untested** — flicker and wrap-corruption are different failure modes in the same rendering layer, so it might or might not. Action: next time a narrow pane is in front of me (phone-reattach), retest the mid-word-split bug under `/tui fullscreen` and record the result here. If fixed, this note moves observed → resolved. The fullscreen recommendation itself is now baked into [`02-stack/01-ai-coding/index.md`](../../../02-stack/01-ai-coding/index.md) § "TUI rendering — turn on fullscreen."

## See also

- [cross-device SSH stack pattern](../../../02-stack/patterns/cross-device-ssh.md) — the workflow that exposes this bug
- [02-stack/01-ai-coding/index.md § TUI rendering](../../../02-stack/01-ai-coding/index.md) — the fullscreen/no-flicker config now recommended stack-wide
