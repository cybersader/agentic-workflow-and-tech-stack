...
## 2026-08-21 addendum — Portagenty as address resolver, not message transport

A source-level review of Portagenty closed one ambiguity in the human-in-the-loop gap above. Portagenty already supplies a useful **control plane**: it registers workspaces, resolves declared sessions, discovers the corresponding tmux or Zellij session, and supports human attach/takeover plus whole-session lifecycle actions. It does **not** currently supply an inter-agent transport. There is no addressed payload delivery, durable queue, acknowledgement, idempotency contract, recipient consumption path, or multiplexer input API.

That distinction suggests a reusable abstraction rather than a convenience command.

### Stable logical address

The durable endpoint should use identity that already survives path movement:
...
`workspace_id` is the registered workspace UUID and `session_name` is the exact declared session name. The sanitized, truncated multiplexer target is a backend detail, not a public identity. Resolving the address should produce a capability-bearing record:
...
This keeps endpoint identity separate from operations such as status, attach, launch, deliver, queue, or receipt inspection.

### Typed delivery contract

A transport-neutral envelope needs enough information for retries, auditing, and safe presentation:
...
`submit_policy` is deliberately separate from insertion. Placing literal text in a composer and pressing Enter are different operations; an adapter must not silently turn a multiline handoff into executed input.

A corresponding receipt should distinguish at least:

```text
accepted | queued | delivered | acknowledged |
expired | rejected | unsupported | failed
```

### Adapter boundary

- **Filesystem inbox:** durable, inspectable, asynchronous, and the default candidate for real agent handoffs.
- **Terminal input:** ephemeral and lossy; available only when a backend advertises safe addressed insertion and submission capabilities. It is not the baseline transport.
- **Local UI or API:** a future presentation/control adapter, not a new source of truth.
- **Happier:** a possible future surface only if its session identity, persistence, trust, and acknowledgement semantics pass the separate evaluation.

The machine-global clipboard is mutable, unaddressed, and unacknowledged. It can remain a human convenience but cannot implement delivery. For the same reason, a one-off `pa paste` command would expose one mechanism while skipping identity, queueing, acknowledgement, retry safety, and backend capability differences; it must not stand in for the abstraction.

This remains research rather than a promoted contract. The existing inter-agent messaging skill and `.claude/ARCHITECTURE.md` should change only after at least one durable adapter is implemented and validated. Resource attribution and granular process control are a separate concern documented in [Agent Resource Supervision and Control Plane](./2026-08-21-agent-resource-supervision-and-control-plane.md).
