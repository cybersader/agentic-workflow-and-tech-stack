...
| Subfolder | Purpose |
|---|---|
| [`zz-inbox/`](./zz-inbox/) | Raw idea capture — hottest surface; quick bullets in `INBOX.md` or per-idea `YYYY-MM-DD-topic.md` files before they've earned a full write-up |
| [`zz-research/`](./zz-research/) | In-progress thinking on principles, patterns, architecture — bullets from the inbox graduate here once they're paragraph-length |
| [`zz-log/`](./zz-log/) | Per-day worklog of significant scaffold changes — `YYYY-MM-DD.md` files appended to as work happens. The narrative thread across many commits ("what direction the day took") rather than per-commit imperative messages. |
| `zz-challenges/` (future — currently lives at [`research/zz-challenges/`](../../research/zz-challenges/)) | Hard open problems that need exploration before they can be decided |

Entries are dated (`YYYY-MM-DD-topic.md` for inbox/research, `YYYY-MM-DD.md` for log) so the evolution of work is navigable chronologically. The [`zz-inbox/INBOX.md`](./zz-inbox/INBOX.md) running list is the one exception — append-a-bullet format, no per-entry frontmatter.
...
- [Philosophy & Principles](../../principles/) — what crystallized research looks like
- [Patterns](../../patterns/) — stack-level patterns (tier 2)
- [Roadmap](../../../ROADMAP.md) — tracks the bigger-picture phases
