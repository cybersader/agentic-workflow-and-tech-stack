...
**The top-level main session, command, or Workflow may orchestrate and delegate. A delegated worker is terminal:** it must not invoke another Agent or Workflow. This is the scaffold's deliberate context and budget policy across tools, not a claim that every runtime is technically incapable of nesting.

```mermaid
flowchart LR
    Cmd["Top-level / Main"] -->|orchestrates| Agent["Delegated worker"]
    Agent -->|loads in-context| Skill["Skill"]
    Agent -->|external tool| MCP["MCP"]
    Agent -.->|FORBIDDEN| Agent2["Another worker"]
...
| Edge | Allowed? |
|---|---|
| Top-level → Worker | yes — the top level owns decomposition and dispatch |
| Worker → Skill | yes — skills load into worker context |
| Worker → MCP | yes — external tools allowed within scope |
| Worker → Worker/Workflow | **NO** — return evidence to the top level instead |

The canonical diagram and full context-funneling explainer live in [`.claude/ARCHITECTURE.md`](./.claude/ARCHITECTURE.md). The short version: **main spawns Worker A → A returns a bounded summary → main decides whether to dispatch Worker B → main synthesizes.** Sequential or top-level-parallel, never recursively delegated.

This is **context funneling** — each worker explores or executes deeply in isolation, returns compressed evidence, and the top-level orchestrator maintains continuity.
...
## CRITICAL: Tailscale SSH browser checks

When a tailnet host uses Tailscale SSH, treat the connection as a `tailscaled` authorization path rather than guest OpenSSH. An online node plus successful `tailscale ping` means an `additional check` prompt is not evidence that `sshd` or the daemon failed.

Use the target operating system's Linux username—not the tailnet account/owner label—and keep one bounded command alive:

```bash
tailscale ssh <linux-user>@<host> 'id && hostname'
```

Surface the emitted `https://login.tailscale.com/a/...` URL as a clickable link, wait for the human to complete browser reauthentication, and verify the same process resumes with the expected identity and hostname. Do not kill it and mint replacement URLs. Do not restart `sshd`, restart `tailscaled`, or reach for VNC/serial unless status/ping or the completed check proves a separate fault. Full pattern: [`02-stack/patterns/cross-device-ssh.md`](./02-stack/patterns/cross-device-ssh.md).
