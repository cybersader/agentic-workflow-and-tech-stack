...
## Near-term evaluations — agent control plane

These are bounded evaluations, not adoption decisions. Each item must satisfy its exit criterion before it can change the installed stack, session source of truth, or network surface.

### Happier mobile control surface

- [ ] Evaluate Happier as an alpha-preview, phone-native companion for local Claude Code sessions.
- [ ] Verify self-hosted relay boundaries, required end-to-end encryption, finite retention, machine-local provider authentication, and direct-session follow versus takeover/import.
- [ ] Prove that unified-terminal sessions preserve the existing launcher preflight, provider/model lock, agent-budget guard, user/project hooks, and local JSONL authority.
- [ ] Compare its real phone workflow against tailnet-constrained remote desktop without installing a GUI, starting a relay, or exposing a port during the research pass.

**Exit criterion:** an isolated, reversible trial plan demonstrates the trust boundary and launcher/guard integration, or the candidate is explicitly rejected. No installation, adoption, credential storage, or source-of-truth transfer is implied by this roadmap entry.

### Addressed inter-agent communication through Portagenty

- [ ] Evaluate Portagenty's stable workspace UUID plus exact declared session name as a logical endpoint address.
- [ ] Define a transport-neutral envelope, capability discovery, idempotency, queue/acknowledgement behavior, and delivery receipts.
- [ ] Use the filesystem mailbox as the durable/auditable baseline; treat terminal input and future UI APIs as optional adapters with explicit capabilities.
- [ ] Reject the system clipboard and a one-off `pa paste` command as the communication architecture.

**Exit criterion:** a Portagenty ADR and synthetic prototype prove addressed delivery and acknowledgement through at least one durable adapter without conflating multiplexer control with message transport.

### Fedora/Linux agent workload supervision

- [ ] Evaluate how a logical Portagenty session can map safely to process trees and systemd cgroup v2 scopes.
- [ ] Compare Plasma System Monitor, Cockpit, `systemd-cgls`, `systemd-cgtop`, Mission Center, and Resources for GUI/process/cgroup coverage.
- [ ] Define resource snapshots that include CPU, memory, swap, I/O, task count, and pressure-stall information—not utilization alone.
- [ ] Define a reuse-safe, confirmation-gated action ladder from agent interrupt through child/process-group/session-scope termination, with multiplexer destruction only as the widest fallback.

**Exit criterion:** the session-to-workload identity model, existing-tool gap analysis, and safe targeting semantics are validated with synthetic non-destructive observation. No custom daemon, GUI, service, process signal, or network endpoint is authorized by this research item.
