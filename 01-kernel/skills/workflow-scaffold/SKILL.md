...
| "Check/update CLAUDE.md after my changes" | `workflow-expert` agent |
| "Audit my existing project's structure" | `workflow-expert` agent |
...
3. Offer: "Want me to spawn `workflow-expert` or `workflow-expert` to check docs?"
...
- "Want me to spawn `workflow-expert` to check if docs need updating?"
